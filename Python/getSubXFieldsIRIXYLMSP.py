...
#------------------------------------------------
outPath='/scratch/kpegion/testSubX/swapdim/python/';
   ...
   for imodel in range(nmodels):
      ...
      for iens in range(nens):
         ...
         for ic in range(nics):
           ...
           if ndims==6:
              data = ncfile.variables[varname][plev,ic,iens,:,:,:]
           elif ndims==5:
           ...
           # Set fillValue
           try:
              tmpdata.setncattr('fillValue',ncfile.variables[varname].getncattr('fillValue'))
           except:
              try:
                 tmpdata.setncattr('fillValue',ncfile.variables[varname].getncattr('missing_value'))
              except:
                 tmpdata.setncattr('fillValue',dfv)
